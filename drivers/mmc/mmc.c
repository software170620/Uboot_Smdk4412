...
if (strncmp(mmc->name, "S5P_MSHC", 8) != 0) {
		for(; count<9;) {
			...
			while (readb(mmc->ioaddr + SDHCI_SOFTWARE_RESET) & (1<<1)) {
				if (timeout_c == 0) {
					debug("CMD Reset is NEVER released\n");
					return 0;
				}
				timeout_c--;
				udelay(1000);
...
//leehc support emmc5.0
	...
	case EXT_CSD_REV_1_6:
			...
			break;
	case EXT_CSD_REV_1_7:
			mmc->version |= EXT_CSD_REV_1_7;
			break;
#if 1 //esshin 20160420 for emmc 128GB(KLMDG8JENB)
	case EXT_CSD_REV_1_8:
			mmc->version |= EXT_CSD_REV_1_8;
			break;
#endif
...
static int mmc_read_ext_csd(struct mmc *host)
{
	...
	printf("mmc_read_ext_csd: ext_csd_rev=%d \n",ext_csd_struct); //esshin

//leehc support emmc5.0
//	if (ext_csd_struct > 6) {
//	if (ext_csd_struct > 7) {
	if (ext_csd_struct > 8) { //esshin 20160420 emmc 128GB(KLMDG8JENB)
