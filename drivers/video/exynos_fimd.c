...
static void exynos_fimd_set_par(unsigned int win_id)
{
	...
	struct exynos4_fb *fimd_ctrl =
		(struct exynos4_fb *)samsung_get_base_fimd();
	unsigned long temp = win_id * 4;

	/* set window control */
	cfg = readl((unsigned int)&fimd_ctrl->wincon0 + EXYNOS_WINCON(win_id));
	...
	writel(cfg, (unsigned int)&fimd_ctrl->vidosd0a + EXYNOS_VIDOSD(win_id));

	cfg = EXYNOS_VIDOSD_RIGHT_X(pvid->vl_col - 1) |
		EXYNOS_VIDOSD_BOTTOM_Y(pvid->vl_row - 1);
	writel(cfg, (unsigned int)&fimd_ctrl->vidosd0b + EXYNOS_VIDOSD(win_id));

	/* set window size for window0*/
	cfg = EXYNOS_VIDOSD_SIZE(pvid->vl_col * pvid->vl_row);
	writel(cfg, (unsigned int)&fimd_ctrl->vidosd0c + EXYNOS_VIDOSD(win_id));
}
...
static void exynos_fimd_set_buffer_address(unsigned int win_id)
{
	...
	debug("%s - start_addr:0x%x, end_addr:0x%x\n", __func__, start_addr, end_addr);
...
static void exynos_fimd_set_clock(vidinfo_t *pvid)
{
	...
	unsigned long pixel_clock, vclk;
	unsigned long src_clock ;
	struct exynos4_fb *fimd_ctrl =
		(struct exynos4_fb *)samsung_get_base_fimd();
	//u64 div64;
...
//	}
	...
	src_clock = (unsigned long)get_lcd_clk();

	pixel_clock = KHZ2PICOS(pixel_clock/1000);
	vclk = PICOS2KHZ(pixel_clock) * 1000;

	cfg = readl(&fimd_ctrl->vidcon0);
	cfg &= ~(EXYNOS_VIDCON0_CLKVALUP_MASK |
			EXYNOS_VIDCON0_VCLKEN_MASK);
	cfg |= (EXYNOS_VIDCON0_CLKVALUP_ALWAYS |
...
//		EXYNOS_VIDCON0_VCLKEN_NORMAL | EXYNOS_VIDCON0_CLKDIR_DIVIDED);
	...
	debug("%s - src_clock:%llu\n", __func__, src_clock );

	if (vclk > MAX_CLOCK) {
		debug("vclk(%d) should be smaller than %d Hz\n",
			src_clock/div, MAX_CLOCK);
	}

	div = src_clock / vclk;

	if (src_clock % vclk) {
		if ((src_clock % vclk) > (vclk/2))
			div++;
	}

	if ((src_clock/div) > MAX_CLOCK)
		debug("vclk(%d) should be smaller than %d Hz\n",
			src_clock/div, MAX_CLOCK);
	...
	debug("%s - remainder_div:%i\n", __func__, remainder_div);
	...
	if (pvid->dual_lcd_enabled)
		div--;
#endif
	cfg &= ~EXYNOS_VIDCON0_CLKVAL_F(0xff);
...
void exynos_fimd_lcd_init(vidinfo_t *vid)
{
	...
	if (vid->interface_mode == FIMD_RGB_INTERFACE) {
		cfg = readl(&fimd_ctrl->vidcon0);
		cfg &= ~EXYNOS_VIDCON0_VIDOUT_MASK;
		...
		cfg &= EXYNOS_VIDCON1_FIXVCLK_MASK;
		cfg |= EXYNOS_VIDCON1_FIXVCLK_VCLK_RUN;
		if (!pvid->vl_clkp)
		...
		/* set timing */
		cfg = EXYNOS_VIDTCON0_VBPDE(pvid->vl_vbpe -1);
		...
		cfg = EXYNOS_VIDTCON1_VFPDE(pvid->vl_vfpe -1);
