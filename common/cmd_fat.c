		...
		int argc, char * const argv[])
{
	...
	offset = simple_strtoul(argv[3], NULL, 16);
	maxsize = simple_strtoul(argv[5], NULL, 16);

	if(maxsize < 0x1200) {
		printf("\n** write size more then 0x1200 **\n");
		return 1;
	}

	

	size = file_fat_write(argv[4], (unsigned char*)offset, maxsize);
	if (size == -1) {
...
U_BOOT_CMD(
	...
	"      to 'dev' on 'interface'\n"
	"      It must be more then at least 4608(0x1200)bytes'\n"
);
#endif
