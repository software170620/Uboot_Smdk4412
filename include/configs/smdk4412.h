...
#define CONFIG_KCPPK_TYPE_MOBILE //#define CONFIG_KCPPK_TYPE_CAR
...
#define CONFIG_EMERGENCY_PROMPT 1
...
#define CONFIG_PM_VDD_G3D	0.0
#define CONFIG_PM_VDD_MIF	1.0
...
/* add i2c test for kcppk */
#define CONFIG_CMD_I2C_TEST
...
*/
#define CONFIG_BOOTCOMMAND3	\
		"setenv loadaddr 0x48000000;" \
		"echo fdisk internal memory to default system 200M user 2891M cache 200M;" \
		"fdisk -c 0;" \
		"echo format mmc 0:2;" \
		"ext3format mmc 0:2;" \
		"echo format mmc 0:3;" \
		...
		"echo format mmc 0:4;" \
		"ext3format mmc 0:4;" \
		"echo fat load bl1_kcppk.bin from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} bl1_kcppk.bin;" \
		"echo flashing bl1_kcppk.bin to internal mmc;" \
		"fastboot flash fwbl1 ${loadaddr};" \
		"echo fat load bl2_kcppk.signed from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} bl2_kcppk.signed;" \
		"echo flashing bl2_kcppk.signed to internal mmc;" \
		"fastboot flash bl2  ${loadaddr};" \
		"echo fat load tzsw_kcppk.signed from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} tzsw_kcppk.signed;" \
		"echo flashing tzsw_kcppk.signed to internal mmc;" \
		"fastboot flash tzsw ${loadaddr};" \
		"echo fat load u-boot.bin from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} u-boot.bin;" \
		"echo flashing u-boot.bin to internal mmc;" \
		"fastboot flash bootloader ${loadaddr};" \
		"echo fat load zImage from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} zImage;" \
		"echo flashing zImage to internal mmc;" \
		"fastboot flash kernel ${loadaddr};" \
		"echo fat load ramdisk-uboot.img from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} ramdisk-uboot.img;" \
		"echo flashing ramdisk-uboot.img to internal mmc;" \
		"fastboot flash ramdisk ${loadaddr};" \
		"echo fat load system.img from mmc 0:1;" \
		"fatload mmc 0:1 ${loadaddr} system.img;" \
		"echo flashing system.img to internal mmc;" \
		"fastboot flash system ${loadaddr};" \
		"echo done...........;" \
		"reset"



#define CONFIG_BOOTCOMMAND4	"fastboot"

#define CONFIG_BOOTCOMMAND5 "ext3format mmc 0:3;ext3format mmc 0:4;"		\
				"movi read kernel 0 40008000;movi read rootfs 0 41000000 100000;bootm 40008000 41000000"

#define CONFIG_BOOTCOMMAND6	\
		"echo fdisk internal memory to default system 200M user 2891M cache 200M;" \
		"fdisk -c 0;" \
		"echo format mmc 0:1;" \
		"fatformat mmc 0:1;" \
...
#define CONFIG_SYS_PROMPT		"VULCAN # "
...
#define CONFIG_IDENT_STRING		" for KCPPK"
