...
/* PMU_DEBUG bits [11:8] = 0x1000 selects XXTI clock source */
#define PMU_DEBUG_XXTI                          0x0800
/* Mask bit[11:8] for xxti clock selection */
#define PMU_DEBUG_CLKOUT_SEL_MASK               0x0E00


#ifndef __ASSEMBLY__
struct exynos4_power {
	unsigned int	om_stat; // 0x0
	unsigned int	lpi_maks0; //0x4
	unsigned int	lpi_maks1; //0x8
	unsigned int	lpi_maks2; //0x0c
	...
	unsigned int	intr_sspread_enable; //0x100
	unsigned int	intr_sspread_use_standbywfi; //0x104
	unsigned int	central_seq_configuration; //0x108
	unsigned int	res2[0xf4];     //0x10c	
	unsigned int	central_seq_option; //0x200
