...
unsigned int default_journal_size(uint32_t block_cnt)
{
	...
	if (block_cnt < 1024 * 1024)
		return (16384);
	if (block_cnt < 2048 * 1024) 
		return (32768); // 16GB 
	if (block_cnt < 4096 * 1024) 
		return (65536); // 32GB 
	if (block_cnt < 8192 * 1024) 
		return (131072); // 64GB 
	if (block_cnt < 16384 * 1024) 
		return (262144); // 128GB 
...
int ext2fs_format(block_dev_desc_t *dev_desc, int part_no, char set_journaling)
{
	...
	uint32_t uuid[4];
	img = malloc(sizeof(__u8) * 4096 *2);
	if (img == NULL) {
		printf("Can't make img buffer~~!!\n");
		return -1;
	}
	uint8_t *img2; /* Group Descriptor Table */
	img2 = malloc(sizeof(__u8) * 4096 * 10);
	if (img2 == NULL) {
		printf("Can't make img2 buffer~~!!\n");
		return -1;
	}
	uint8_t *reserve_img;
	reserve_img = malloc(sizeof(__u8) * 4096 *10);
	if (reserve_img == NULL) {
	...
	img3 = malloc(sizeof(__u8) * 4096 *20 );
	if (img3 == NULL) {
		printf("Can't make img3 buffer~~!!\n");
		return -1;
	}
	uint8_t *img4;
	img4 = malloc(sizeof(__u8) * 4096 * 10);
	if (img4 == NULL) {
	...
	uint8_t *img5;
	img5 = malloc(sizeof(__u8) * 2048 * 10);
	if (img5 == NULL) {
		printf("Can't make img5 buffer~~!!\n");
		return -1;
	}
	uint8_t *rootdata = malloc(sizeof(__u8) * 4096 *10);
	if (rootdata == NULL) {
		printf("Can't make rootdata buffer~~!!\n");
		return -1;
	}
	uint8_t *inode_data = malloc(sizeof(__u8) * 4096 * 10);
	if (inode_data == NULL) {
		printf("Can't make inodedata buffer~~!!\n");
		return -1;
	}

	uint8_t *inode_data2 = malloc(sizeof(__u8) * 4096 * 10);
	if (inode_data2 == NULL) {
	...
	for (bgcnt = 0; bgcnt < blk_group_num; bgcnt++) {
		...
		memset(img, 0x0, sizeof(__u8) * 4096 * 2);
		if (bgcnt == 0)
		...
		memset(img2, 0x0, sizeof(__u8) * 4096 * 10);
		...
		if (set_journaling) {
			...
			if (default_jnl_blocks == 1024)
				...
				temp_val = 0x8000000;
			else if (default_jnl_blocks == 65536) 
				temp_val = 0x16000000; 
			else if (default_jnl_blocks == 131072) 
				temp_val = 0x32000000; 
			else if (default_jnl_blocks == 262144) 
				temp_val = 0x64000000; 
			else if (default_jnl_blocks == 524288) 
				temp_val = 0x128000000; 
		...
		*/
		...
		if (have_rev_gdt) {
			start_block += 8;
			if(blk_group_num<128){
				blksize=8;
			}else{
				blksize=(((blk_group_num/128)+1) * 8);
			}
			if (dev_desc->block_write(dev_desc->dev, start_block, blksize,
				(ulong *)img2) != blksize) {
				printf("Can't write Descriptor Table(%d)~~~!!!\n", bgcnt);
			}
			
			start_block += blksize-8 ;
		}

		/* Write reserved region */
		if (bgcnt == 0) {
			memset(reserve_img, 0x0, sizeof(__u8) * 4096 * 10);
		...
		if (bgcnt != blk_group_num - 1) {
			memset(img3, 0x00, sizeof(__u8) * 4096 *20);
			...
			memset(img3, 0xff, sizeof(__u8) * 4096 * 20);
		...
		*/
		...
		memset(img4, 0xff, sizeof(__u8) * 4096 * 10);
		...
		*/
		...
		memset(img5, 0x00, sizeof(__u8) * 2048 * 10);
		...
		if (bgcnt == 0) {
			/* root data */
			memset(rootdata, 0x00, sizeof(__u8) * 4096 * 10);
			mk4(rootdata, 0x2);
			...
			/* root + 1 data */
			root_data_blk++;
			memset(inode_data, 0x00, sizeof(__u8) * 4096 * 10);
			mk4(inode_data, 0xb);
			...
			if (bgcnt == 0) {
				...
				if (remain_blocks != 0) {
					...
					for (j=0;j < needed_blocks; j++) {
						...
						memset(inode_data2, 0x00, sizeof(__u8) * 4096 * 10);
						for (k = 0; k < 1024; k++) {
