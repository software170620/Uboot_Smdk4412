...
{	
	...
	//printf("%s - start block:%i, getsize:%i \n", __func__, startblock, getsize);
	     ...
	     unsigned long size)
{
	...
	//printf("gc - clustnum: %d, startsect: %d\n", clustnum, startsect);

	nr_block = size / FS_BLOCK_SIZE;
	      ...
	      unsigned long maxsize)
{
	...
	do {
		...
		//LCD_setprogress((actsize *100)/total_filesize);	
		//printf("2 persentage:%i\n", (actsize *100)/total_filesize);
		...
		LCD_setprogress(PERSENTAGE(gotsize, 100, total_filesize));
		if (get_cluster(mydata, endclust, buffer, (int)actsize) != 0) {
			printf("Error reading cluster\n");
			return -1;
		}
		gotsize += actsize;
		//LCD_setprogress((gotsize *100)/total_filesize);	
		LCD_setprogress(PERSENTAGE(gotsize, 100, total_filesize));
		return gotsize;
getit:
		if (get_cluster(mydata, curclust, buffer, (int)actsize) != 0) {
