...
static int disk_write(__u32 block, __u32 nr_blocks, void *buf)
{
	...
	//debug("%s - start block:%i, getsize:%i \n", __func__, cur_part_info.start + block, nr_blocks);

	//printf("%s - start block:%i, nr_block:%i\n", __func__, cur_part_info.start + block, nr_blocks);
...
static __u8 mkcksum2(const char name[8], const char ext[3])
{
	int i;
...
fill_dir_slot(fsdata *mydata, dir_entry **dentptr, const char *l_name)
{
	...
	checksum = mkcksum2((*dentptr)->name, (*dentptr)->ext);
...
//	printf("size: %d , mydata->sect_size:%i \n", size, mydata->sect_size);

	nr_block = size / mydata->sect_size;
	if(nr_block == 0 && size > 0) nr_block = 1;
...
static void flush_dir_table(fsdata *mydata, dir_entry **dentptr)
{
	...
	if (set_cluster(mydata, dir_curclust,
		    ...
		    mydata->clust_size * mydata->sect_size) != 0) {
		debug("error: wrinting directory entry\n");
		return;
	}
 ...
 */
 #define PERSENTAGE(a,b,c) (unsigned int)(unsigned long long )(((unsigned long long)a*(unsigned long long)b)/(unsigned long long)c)
static int
	      ...
	      unsigned long maxsize)
{
	...
	unsigned long total_filesize = filesize;
	...
	endclust = curclust;
	debug("actsize:%ld bytes, bytesperclust:%ld\n", actsize, bytesperclust);	
	...
	do {
		...
		/* actsize >= file size */
		actsize -= bytesperclust;	
	//	printf("1  actsize: %i ---	gotsize:%i\n", actsize, gotsize);

		/* set remaining clusters */
		if (set_cluster(mydata, curclust, buffer, (int)actsize) != 0) {
			debug("error: writing cluster 1\n");
			return -1;
		}
		/* set remaining bytes */
		gotsize += (int)actsize;
		...
		buffer += actsize;
		actsize = filesize;
	//	printf("2  actsize: %i ---  gotsize:%i\n", actsize, gotsize);
		LCD_setwriteprogress(PERSENTAGE(gotsize, 100, total_filesize));
		if (set_cluster(mydata, endclust, buffer, (int)actsize) != 0) {
			printf("error: writing cluster 2\n");
			return -1;
		}
		gotsize += actsize;
	//	printf("3  actsize: %i ---  gotsize:%i\n", actsize, gotsize);
	...
	unsigned long size)
{
	...
	//printf("%s sect_size:%i\n", __func__, mydata->sect_size);
	...
	*/
	...
	if (disk_read(cursect,
		(mydata->fatsize == 32) ?
		(mydata->clust_size) :
		LINEAR_PREFETCH_SIZE / mydata->sect_size, do_fat_read_block) < 0) {
...
//	goto exit;
	...
	if (retdent) {
		...
		if (ret) {
			debug("Error: %ld overflow\n", size);
			goto exit;
		}

		ret = clear_fatent(mydata, start_cluster);
		if (ret) {
			debug("Error: clearing FAT entries\n");
			goto exit;
		}

		ret = set_contents(mydata, retdent, buffer, size);
		if (ret < 0) {
			debug("Error: writing contents\n");
			goto exit;
		}
		write_size = ret;
		...
		if (ret < 0) {
			debug("Error: finding empty cluster\n");
			goto exit;
		}
